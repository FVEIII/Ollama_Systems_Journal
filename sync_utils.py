...
def sync_csv_to_yaml(csv_path):
    updated_rows = []

    with open(csv_path, "r", newline="", encoding="utf-8") as csvfile:
        reader = csv.DictReader(csvfile)
        for row in reader:
            if not row.get(KEY_FILE_PATH):  # Skip malformed rows
                continue
            ...
            for key, value in row.items():
                ...
                if isinstance(old_value, list): old_value = sorted(old_value)
                if isinstance(new_value, list): new_value = sorted(new_value)
            ...
            if changes:
                for key, value in row.items():
                    if key == KEY_FILE_PATH:
                        continue
                    try:
                        frontmatter[key] = eval(value) if key == KEY_RELATED_LOOPS and value else (value or None)
                    except Exception:
                        frontmatter[key] = []
